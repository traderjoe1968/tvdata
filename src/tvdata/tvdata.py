...
class TvDatafeed:
    ...
    def __init__(
        ...
        self.ws_debug = False
        self._userdata = {"username":username, "password":password, "token":None}

        self._userdata_file = user_data_dir(appname="tvdata", roaming=True)
        os.makedirs(os.path.dirname(self._userdata_file), exist_ok=True)
        try:
            with open(self._userdata_file,'r') as f:
                self._userdata = toml.loads(f.read())
        except IOError:
            self.token = None
        
        if self.token is None:  
            self.token = self.__auth(self._userdata["username"], self._userdata["password"])
    ...
    @staticmethod
    def __getcode():
        logging.debug("Asking user for 2FA code")
        # code = easygui.enterbox("Enter 2FA code:")
        code = input("Enter 2FA code: ")
        return int(code)
        
    @property
    def token(self):
        return self._userdata["token"]

    @token.setter
    def token(self, value):
        self._userdata["token"] = value
        with open(self._userdata_file, 'w') as f:
            toml.dump(self._userdata, f)
